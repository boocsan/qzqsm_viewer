import { getValueNumber, datetimeToString } from "../string_converter";

export default function EarthquakeEarlyWarningParser(sentence: string): string {
  const pl = sentence.slice(130, 210);
  const plList: string[] = [];
  for (let i = 0; i < 80; i++) {
    if (pl[i] === "1") plList.push(Pl[i]);
  }
  ...
  const t = {
    ...
    Ll: getValueNumber(sentence, 122, 4),
    Ul: getValueNumber(sentence, 126, 4),
    Pl: pl
  };
  ...
  const ep: string = Ep(t.Ep);
  const de: string = De(t.De);
  const ma: string = Ma(t.Ma);
  const under: string = In(t.Ll);
  const upper: string = In(t.Ul);

  const text = `緊急地震速報
${co}
発表時刻：${datetimeToString(publishedAt, "{MM}月{dd}日{HH}時{mm}分")}
震央地名：${ep}
地震発生時刻：${datetimeToString(originTime, "{dd}日{HH}時{mm}分")}
深さ(km)：${de}
マグニチュード：${ma}
震度(下限)：${under}
震度(上限)：${upper}
...
function In(code: number): string {
  switch (code) {
    case 1: return "震度0";
    case 2: return "震度1";
    case 3: return "震度2";
